...
-- Title options -- 
* From noise to signal: Separating problem-skipping from accuracy in student ability estimation
...
17/03/2026 - alt. introduction outline
* general introduction 
    * 0.1 we see problem-skipping in ALS; zero matters argument in short
    * 0.2 this complicates measurement because most models are unidimensional 
* section 1: problem-skipping and evidence of it as a separate latent trait 
    * 1.1 introducing problem-skipping is a design consideration (positive, but also assumes some association to proficiency)
    * 1.2 previous lit problem-skipping <-> task-proficiency 
    * 1.3 metacognition 
* section 2: measurement consequences 
    * unidimensional models power adaptivity 
    * problem-skipping threatens unidimensionality and may introduce bias 
    * previous lit bias
* section 3: aims & research questions
    * modeling problem-skipping as separate latent trait necessary
    * done previously in pen-and-paper testing (cite previous lit)
    * research gap: unclear whether multidimensional structure appears in adaptive learning, where problem-skipping is embedded in design 
    * state rqs
    * implications (fairness / accuracy)
\end{comment}
...
%%% Suggestions Alexander
% Par 1. Utility of online data for estimating student ability & importance of accurate measurement models 
%For the sake of better understanding and improving student learning, education relies on measurement models that accurately assess student ability. Measurement models underlying large-scale personalized learning systems are primarily unidimensional, \textit{i.e.}, the primary observations driving the latent trait estimation are whether or not a response was correct (here referred to as accuracy [for example, ref]). However, many platforms offer response options beyond simply giving the answer, such as requesting hints or skipping questions (here referred to as problem-skipping). While such behaviors have been used in the past to predict performance (ref), the latent traits underlying problem-skipping remain unclear. This study explores whether skipped responses represent a distinct cognitive process, separate from accuracy, in estimating learner ability. 
...
% End this par with problem statement 
...
% https://doi.org/10.3758/s13428-022-01953-x
% 


[rewrite: ALS, (student model) psychometrics, irt, rasch model, formula, person and item parameters] Adaptive learning systems estimate learner proficiency from response data using latent variables models such as item response theory (IRT) [ref] and Bayesian knowledge tracing (BKT)[ref] or models inspired by cognitive architectures (e.g, memorylab). Although these frameworks differ in their formulation[The core assumption of approach is that..], they share a common assumption: observable performance is driven by a latent dimension representing task proficiency. Such unidimensional measurement models have fundamentally transformed educational measurement in practice [ref]. They offer a parsimonious and interpretable framework for representing student proficiency~\parencite{rasch1993probabilistic, hambleton1991fundamentals, embretson2013item, de2013theory} and under adequate model fit, they enable the comparison of individuals' ability regardless of which specific items from a calibration pool that these students respond to~\parencite{rasch1993probabilistic, wright1982rating}. This modeling characteristic has been ground-breaking for large-scale assessment~\parencite{lord2012applications, van2010elements} [add example of ed-tech]. With these tools, educators and test constructors can identify poorly functioning items while maintaining robustness against, for example, missing data~\parencite{mislevy1996missing, de2013theory}.  
...
\subsection{Non-responding: A Meta-Cognitive Perspective} % interpretation? view? perspective? explanation? 
...
As discussed, finding evidence for multidimensionality would imply both ability assesment  and adaptive task selection could be improved. In addition, fitting the IRTree model to response data allows us to examine the latent correlations between the nodes for items and students. This would help shed light on the currently sparse findings concerning the relationship between accuracy and problem-skipping: 
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/series.jpeg}
...
\begin{figure}[htbp]
      \centering
      \includegraphics[width=0.5\linewidth]{figures/grade_diff.png}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/rt.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/cor.pdf}
...
\begin{figure*}[t]
    ...
    \includegraphics[width=\textwidth,height=6cm]{figures/user_interactions.pdf}
