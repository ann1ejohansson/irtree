...
%\title{From noise to signal: A multidimensional approach to modeling problem-skipping in online learning}
\title{From noise to signal: Separating problem-skipping from accuracy in student ability estimation}
%\title{Meta-cognition matters for measurement: Separating problem-skipping from accuracy in estimating student ability}
%\title{Unpacking problem-skipping in student performance models using IRTrees}
...
New outline of introduction: 
1. Goal of EDM is to use large-scale educational log data to infer bheavioral patterns in learning, with the utlimate goal of estimating when mastery of a specific subject is achieved. For this purpose, models that properly assess learner ability are crucical. 
2. There is a potential to use information beyond the correctness of a response. Many environments include some type of problem-skipping/help-seeking option (here we focus on problem-skipping / "formal help seeking"). However, these responses are often treated as incorrect or missing. (
- beyond modeling the correctness of a response, there is an increasing recognition for the need to model other process-related behaviors, such as "lowering or changing learning aspirations" (Yang & Stefaniak, 2023). 
3. Monitoring and regulating cogntive processes (metacognition) plays a crucial role in successful adaptive learning. Students need to self-assess their likelihood of success on a given problem and allocate their effort accordingly. Here, explain how explicitly modeling problem-skipping sheds a light into meta cognitive processes, allowing us to capture dimension of self-regulated learning that traditional accuracy-based metrics commonly overlook. 
4. Literature review on the previous relations between problem-skipping and accuracy in online learning systems (hint requests correlate negatively with performance; positively with errors; students that need the most help show low amount of help-seeking strateies). Also connect with gaming the system, and disengaged behavior. Fan (2023) - review noticed a need find learning patterns where students lower or change their learning aspirations. 
5. Explain the use of the IRT approach in modeling student ability, and the disadvantages of the unidimensional model. Previous research has shown that ignoring or treating qm-responses as incorrect leads to bias in ability estimates.
- Explain similarity of IRT with BKT. But, core difference is that BKT still incorporate QM-responding in the prediction of accuracy, whereas we estimate it separately. 
- Explanation of IRTree model as a solution. 
- Previous success in modeling student data with IRTree (Abes study - separating response times in online learning; previous studies using school testing data found correlations of problem-skipping and ability using IRTree approach). 
6. Aims of the current study, RQs. 

TO-DO:
1. Introduction:
    - [✓] restructure with new outline 
    - [✓] add all references
    - [✓] improve equations?
2. Methods:
...
4. Discussion
    ...
    - [✓] change up limitations?
5. Formatting
    - [✓] correct formatting references (only \cite can be used)
    - [✓] anonymize learning platform
...
Deciding whether to skip a problem or not requires the student to self-assess their likelihood of success and allocate their effort accordingly \cite{winne2017cognition}. This meta-cognitive process, consisting of monitoring and regulation, is crucial for successful learning \cite{rabinowitz2017nteraction, fleur2021metacognition, colantonio2024predicting}, particularly in online settings \cite{zhao2020time, kim2009not, fidan2024emotions, shields2024active}. The extent to which meta-cognitive tools are used differs across learners, as a function of individual differences in, for example, prior knowledge \cite{taub2014can}, motivation \cite{sungur2007modeling, karlen2016differences}, and anxiety \cite{matthews1999metacognition, wells1995metacognition}. Differential patterns in problem-skipping may also be induced by avoidance strategies stemming from reduced effort or affective states, such as anxiety and boredom, which influence engagement and self-regulation through their impact on motivation and perceived control \cite{pekrun2013control}. Compounding this issue, students do not always know that they need help \cite{aleven2000limitations}. If individual differences in the use of meta-cognitive strategies are ignored, systemic patterns of problem-skipping behavior underlying such traits could lead to confounds in ability estimates. By explicitly modeling problem-skipping behavior, we capture an important dimension of self-regulated learning that traditional unidimensional models overlook. 

% Par 3. Help-seeking / Problem-Skipping literature
Research on problem-skipping in online learning is sparse, and existing studies reveal a mixed portrait of its relationship with accuracy. On the one hand, results from a large-scale intervention study demonstrated that limiting access to a problem-skipping option increased effortful practice and improved learning in an online mathematics learning platform \cite{savi2018delaying, savi2023combating}. Hint-requests have also been found to correlate negatively with pre- and post- mathematics test scores, and positively with mathematical errors during online learning \cite{norum2024student}. These studies point to a negative relationship between problem-skipping and accuracy. On the other hand, in the same study \cite{norum2024student}, the authors found evidence for individual differences in the extent to which the hint-request option was used, and highlighted that a proportion of students which were identified to need the most help relied the least on help-seeking strategies. Similarly, an eye-tracking study found that students with lower accuracy paid less attention to hints \cite{conati2013understanding}. Further studies have found evidence for individual differences in hint-processing but show that these individual differences are not related to learning gains \cite{goldin2012learner, goldin2013hints}. Further, studies aiming to classify online learning behavior as ``off-task'' or disengaged \cite{baker2004off, baker2004detecting, baker2005performance, beck2004using} reveal a group of students who misuse response options such as skipping. These findings suggest that problem-skipping behaviors may not be explained by student ability alone. 

% Par 4 Explanation IRT and connection to BKT. 
\subsection{Unidimensional measurement models}
Problem-skipping has previously been incorporated into ability assessment, largely within Bayesian Knowledge Tracing (BKT) frameworks. Since its introduction \cite{corbett1995knowledge}, where problem-skipping behavior is treated the same as an incorrect response, there have been several extensions which treat problem-skipping differently. For example, the partial-credit model \cite{wang2010representing, wang2013extending, ostrow2015improving}, gives some, but not full, credit for hint-seeking or multiple attempts to a problem. Other implementations include incorporating the history of help-seeking to improve the prediction accuracy of guesses and slips \cite{baker2008more} or detecting affective states \cite{corrigan2015dynamic}. A comprehensive overview of proposed enhancements to the classical BKT model is provided by \cite{saric-grgic2024twenty-five}. 

Another commonly used measurement model for assessing student ability is Item Response Theory (IRT). In this model, student ability is assessed based on the likelihood of a correct response, given specific properties of the current test item, and the student's current ability. In this paper, we use extensions of the 1-Parameter Logistic Model (1PL; also referred to as the Rasch Model \cite{rasch1993probabilistic}) to account for hierarchical decision processes. In the 1PL model, the probability that a student with ability $\theta_p$ answers item $i$ correctly is given by the function: 
\begin{equation}
  P(X_i = 1 \mid \theta_p, \beta_i) = \frac{1}{1 + e^{-(\theta_p - \beta_i)}}
  \label{eq:Rasch}
\end{equation}
where $\beta_i$ is the difficulty parameter for item $i$, which represents at what ability level the student has a 50\% chance of responding to the item correctly. In BKT terms, the $\theta$ parameter (student ability) represents the probability of moving from an unlearned to a learned state, and the $\beta$ parameter (item difficulty) represents an item-specific probability of moving from a learned state back into an unlearned state. This bridge between IRT and BKT models of student ability has been formalized by \cite{deonovic2018learning}. Although the current paper deals with data obtained under an IRT measurement model, results will be relevant for online learning systems utilizing a BKT-framework. 

Importantly, IRT and BKT models share the feature that ability is measured on a unidimensional scale. Both measurement models thus assume that problem-skipping is driven by the same latent trait as the accuracy of a response. Previous work has demonstrated that ignoring missing responses in IRT approaches can lead to bias on person- and item-parameter estimates \cite{finch2008estimation, mislevy1996missing, pohl2014dealing, holman2005modelling, debeer2017modeling}. Similarly, prediction accuracy was found to increase in BKT models when disengaged responses (including long sequences of skipped problems) were excluded from the ability estimation \cite{gorgun2022considering}. These findings support the notion that problem-skipping might be indicative of a separate meta-cognitive state which is not directly related to ability, and signal a need to test the assumption that problem-skipping and accuracy can be measured on the same dimension. 
...
Previous work utilizing IRTree models has found support for problem-skipping and accuracy as distinct processes in educational assessment data obtained from traditional pen-and-paper tests \cite{okumura2014empirical, pohl2014dealing, debeer2017modeling, huang2020mixture, holman2005modelling}. A simulation study found that a 1PL model results in bias in person- and item-parameters when data is missing not at random, and that modeling the missingness mechanism with an IRTree model significantly reduces this bias \cite{debeer2017modeling}. This was especially the case for students who skip many responses. Empirical studies on PISA reading proficiency tests in Argentina \cite{debeer2017modeling} and Japan \cite{okumura2014empirical}, and reading and mathematical competence tests in German fifth grade students students' \cite{pohl2014dealing}, found that performance was negatively related to their skipping propensity, and modeling reading proficiency with a IRTree vs.\ 1PL model resulted in systematic differences in parameter estimation. The tree model was the best fit to the data, highlighting that missing responses in educational achievement data should not be ignored. Additionally, IRTrees have been successfully applied to online learning data by accounting for the speed-accuracy trade off in ability estimation \cite{coomans2016distinguishing, hofman2018fast}, but this approach did not take problem-skipping into account. 
...
\noindent This is the formalization of the explicit scoring rule (Figure~\ref{fig:scoring}). It is a so-called high speed, high stakes (HSHS) scoring rule \cite{maris2012speed}, which imposes a speed accuracy trade-off in response behavior. The discrimination parameter, $\alpha_i$, is equal to $1/d$. In this way, the model maintains a constant discrimination value across items, and thus the calculated expected score is equivalent to a 1PL IRT model: 
...
\begin{table*}[t]
    ...
    \begin{tabular}{llccccc}
        \hline
        \makecell{\textbf{Model}} & \makecell{\textbf{Random Parameters}} & \makecell{\textbf{AIC}} & \makecell{\textbf{BIC}} & \makecell{\textbf{RMSE}} & \makecell{\textbf{cor($\theta^{(1)}, \theta^{(2)})$}} & \makecell{\textbf{cor($\beta^{(1)}, \beta^{(2)})$}}\\
        \hline
        Fully estimated IRTree & $\theta_p, \theta_i, \beta_p, \beta_i$ & $1519007$ & $1519095$ & 0.3217 & $0.41$ & $0.78$ \\
        Item-constrained IRTree & $\theta_p, \beta_p, \theta_i = \beta_i$ & $1533720$ & $1533783$ & 0.3234 & $0.35$ & - \\
        User-constrained IRTree & $\theta_i, \beta_i, \theta_p = \beta_p$ & $1550148$ & $1550211$ & 0.3251 & - & $0.73$ \\
        Fully constrained IRTree & $\theta_i = \beta_i, \theta_p = \beta_p$ & $1566326$ & $1566377$ & 0.3268 & - & - \\
        \hline
    \end{tabular}
...
\begin{figure*}[t]
    ...
    \includegraphics[width=\textwidth,height=6cm]{user_interactions.pdf}
...
Our findings extend previous work on response scale- and achievement test data---demonstrating that ignoring skipped items may lead to bias in the measurement of latent traits \cite{holman2005modelling, pohl2014dealing, huang2020mixture, okumura2014empirical, debeer2017modeling}---to online learning platforms. They also shed some light on the mixed evidence concerning the relationship between problem-skipping and accuracy, adding to the findings of \cite{norum2024student} that more problem-skipping is associated with more errors, and the body of literature indicating individual differences in the use of alternative response options \cite{norum2024student, goldin2012learner, goldin2013hints}. However, the evidence that problem-skipping and accuracy are distinct and only moderately related may also help to explain the discord in the literature. Specifically, the imperfect relationship suggests that the observed association between problem-skipping and accuracy may be influenced by contextual factors, such as the design of the learning environment or task difficulty, which give space for different manifestations of meta-cognitive strategies and motivational states such as frustration or boredom. Future work should aim to disentangle these contextual influences to better understand the role of problem-skipping in learning and assessment. 
