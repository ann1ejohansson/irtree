...
```{r}
# Source the prep script to apply cleaning (grade filter, derived columns),
# reshape to long format, and filter to eligible users.
# Produces: logs_clean (wide, eligible users) and dat_irt (long, for modelling).
...
dt <- as.data.table(get(data_object))
dt[, question_mark  := as.integer(answer == "¿")]
dt[, response_type  := ifelse(question_mark == 1, "qm",
                               ifelse(correct_answered == 0, "error", "correct"))]
dt <- dt[grade >= min_grade & grade <= max_grade]
...
library(DescTools)
dt_t <- dt[t < 20, Freq(response_type), t][order(t)]
...
dt_session <- dt[new_user_domain_modified_count > 0 & new_user_domain_modified_count < 100, Freq(response_type), new_user_domain_modified_count][order(new_user_domain_modified_count)]
...
combined_plot
save_plot(combined_plot, name = "combined_plot", height = 4, width = 6)
```
